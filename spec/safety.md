...
| Tool Authorization Gate | Bridge, API, file, credential, process, or tool request. | Tool name, requested action, authorization scope, credentials, tool version. | `allow`, `delay`, `review`, `block`, `terminate` | Tool decision, scope reference, denied action if any. | Missing or mismatched authorization scope. | Route scope mismatch to Objective and Scope Integrity Gate. |
...
| Relational and Psychological Safety Gate | Sustained relational interaction, crisis signal, dependency signal, delusion/mania trajectory, or synthetic intimacy. | Conversation trajectory, risk signals, persona output, monitor output, memory classification. | `allow`, `delay`, `review`, `block`, `terminate`, `quarantine` | Trajectory assessment, crisis record, relational safety event. | Critical combination without safe transition or monitor evidence. | Stop reinforcing continuation and escalate to human or emergency support procedure defined by implementation. |
...
| Post-Action Assurance Gate | Mission completion, incident, model update, tool update, threshold change, or conformance claim. | Evidence record, final result, residual risk, denied actions, reviewer decisions. | `allow`, `delay`, `review`, `block`, `quarantine`, `invalidate` | Assessment status, residual risk, reassessment trigger, corrective action. | Evidence missing, result invalid, or residual risk not declared. | Invalidate conformance claim until reassessment is complete. |
